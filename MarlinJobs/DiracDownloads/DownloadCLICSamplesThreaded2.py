...
eventsToDownload = [
#                       { 'EventType': 'ee_nunuqqqq', 'Energy':  1400, 'DetectorModel':'clic_ild_cdr', 'ReconstructionVariant':'clic_ild_cdr_ggHadBkg', 'AnalysisTag': 14, 'ProdID': 5527, 'NumberOfEvents': 335300 },
...
for eventSelection in eventsToDownload:
    ...
    lfns = res['Value']
    fileFormat = 'ProdID_' + str(prodID) + '_' + eventType + '_' + str(energy) + 'GeV_Tag' + str(analysisTag) + '_(\d+)_Of_(\d+)(.*?).root'
    ...
    for lfn in lfns:
        ...
        fineLocalPath = os.path.join(localPath, matchObj.group(3))
